...
function Card({ title, id, position, activeProjectNameListCardCollection, listID }) {
	...
	const handleBlurInput = async () => {
		if (inputValue.trim() !== '') {
			await activeProjectNameListCardCollection.doc(id).update({
			...
			{provided => (
				...
				>
					<Paper
						...
						className='m-1 border-[1px] border-gray-300 hover:bg-gray-300 rounded-md flex flex-col'
					>
						{labels && (
							<div className='flex flex-grow flex-wrap justify-start items-start'>
								{labels?.docs.map(label => (
									<Label
										...
										menu={false}
										activeProjectNameListCardLabelsCollection={null}
									/>
								))}
							</div>
						)}
						<div className='flex flex-grow w-full items-center'>
							...
							<IconButton
								...
								className='hover:bg-gray-400 p-[0.5rem] focus:outline-none h-full'
							>
								<EditIcon />
							</IconButton>
						</div>
						<div className='flex'>
							{cardMenu && (
								<div
									id='card__menu__root'
									tabIndex={-1}
									className='text-gray-800 flex items-center border-t-2 border-gray-300 w-full h-[2.7rem] shadow-lg focus:outline-none'
								>
									...
									<LabelIcon
										...
										className='flex-grow h-full bg-indigo-500 hover:bg-indigo-600'
									/>
									<DeleteIcon
										onClick={() =>
											deleteCard(state.userEmail, state.activeProject, listID, id, position)
										}
										className='bg-red-500 rounded-br-md flex-grow h-full focus:outline-none hover:bg-red-600'
									/>
								</div>
							)}
						</div>
						<div className='flex'>
							{showLabels && (
								<Suspense fallback={<LoadingElement />}>
									<Labels
										activeProjectNameListCardLabelsCollection={activeProjectNameListCardCollection
											?.doc(id)
											.collection('labels')}
									/>
								</Suspense>
