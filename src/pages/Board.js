...
function Board({ location }) {
	...
	const [collaborationUsers] = useCollection(
		db
			.collection('users')
			.doc(state.userEmail)
			.collection(state.userEmail)
			.doc(projectID)
				...
				await db
					...
					.then(async docSnapshot => {
						if (docSnapshot?.exists) {
							...
							setActiveProjectNameListsCollection(
								db
									.collection('users')
									.doc(state.userEmail)
									.collection(state.userEmail)
							...
							await db
								...
								.doc(state.userEmail)
								.get()
								.then(doc => setAuthorImageURL(doc.data().userImageURL))
						} else {
							...
							<div ref={provided.innerRef} {...provided.droppableProps} className='flex'>
								{lists?.docs.map(doc => (
									<Suspense fallback={<LoadingElement />} key={doc.id}>
										<List
											listPosition={doc.data().position}
											listID={doc.id}
											title={doc.data().title}
											activeProjectNameListsCollection={activeProjectNameListsCollection}
										/>
									</Suspense>
								))}
